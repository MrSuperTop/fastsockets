...
HandlerResponse = BaseMessage | Coroutine[BaseMessage, Any, Any] | None
HandleFunction = Callable[P, Coroutine[HandlerResponse | None, Any, Any]]
...
class BaseActionHandler(ABC, Generic[ValidData, ResponseMessage]):
    def __init__(
        ...
        custom_handler_function: Callable[
            ..., Coroutine[ResponseMessage | None, Any, Any]
        ] | None = None
    ) -> None:
        self.action = action
        ...
        # TODO: This should be abstracted away. Another layer which represents any handler has to implemented and a separate class from which the class handlers will inherit
        self.custom_handler_function = custom_handler_function
    ...
    async def __call__(self, validated_data: ValidData) -> ResponseMessage | None:
        ...
        if self.custom_handler_function is not None:
            result = await self.custom_handler_function(**needed_data)
        else:
